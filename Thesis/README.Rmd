...
opt_port_return <- function(data, method, cov_calc_period = 100) {
  ...
  weights <- data %>% 
    ...
    mutate(weight = case_when(
      ...
      method == "minvar" ~ optimalPortfolio(Sigma = cov, 
                                            control = list(type = "minvol", constraint = "lo")),
      method == "invvol" ~ optimalPortfolio(Sigma = cov, 
                                            control = list(type = "invvol", constraint = "lo")),
      method == "erc" ~ optimalPortfolio(Sigma = cov_ht, 
                                         control = list(type = "erc", constraint = "lo")),
      method == "maxdiv" ~ optimalPortfolio(Sigma = cov_ht, 
                                      control = list(type = "maxdiv", constraint = "lo"))
      )) %>% 
    select(date, Asset, weight) %>% 
    tbl_xts(cols_to_xts = "weight", spread_by = "Asset")
  ...
  data_expost_xts <- data %>%
    ...
    spread(Asset, Return) %>% 
    arrange(date) %>% 
    filter(date>first(date) %m+% days(80)) %>% 
    tbl_xts()
...
naive_port_m1 <- market_1 %>% 
  ...
  map_dfr(~opt_port_return(.x, method = "naive"), .id = "Universe")

min_var_port_m1 <- market_1 %>% 
  split(market_1$Universe) %>%
  map_dfr(~opt_port_return(.x, method = "minvar"), .id = "Universe")
...
```{r}
average_drawdown <- function(portfolio_returns) {
  naive_port_m1 %>% 
  split(naive_port_m1$Universe) %>% 
  map(~select(.data = .x, date, portfolio.returns)) %>% 
  map(~tbl_xts(.x)) %>% 
  map(~AverageDrawdown(.x)) %>% 
  map_dfr(~as_tibble(t(.x))) 
}

average_drawdown(naive_port)
...
naive_port_m1 %>% 
  ...
  map(~tbl_xts(.x)) %>% 
  map(~SharpeRatio(.x)) %>% 
  map_dfr(~as_tibble(t(.x)), .id = "Universe")
