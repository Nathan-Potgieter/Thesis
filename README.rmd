...
enc_m3 <- list(naive = naive_enc_m3, # saving calculation time
               minvol = minvol_enc_m3,
               invvol = invvol_enc_m3,
               erc = erc_enc_m3,
               maxdiv = maxdiv_enc_m3)
save(enc_m3, file = "data/enc_m3.rda")
...
save(enc_m4, file = "data/enc_m4.rda")
...
save(enc_m5, file = "data/enc_m5.rda")
...
## Building normalized data tables for each portfolio
```{r eval=FALSE, include=TRUE}
normalize <- function(risk_metrics) {
  risk_metrics %>%
    gather(portfolio, value,-Metric) %>%
    group_by(Metric) %>%
    mutate(normalized = (value - mean(value)) / sd(value)) %>%
    select(Metric, portfolio, normalized) %>%
    spread(portfolio, normalized) %>% mutate(
      order = case_when(
        Metric == "Sharp" ~ 1,
        Metric == "SD" ~ 2,
        Metric == "Downside Deviation" ~ 3,
        Metric == "VaR" ~ 4
      )
    ) %>% arrange(order)
}
...
norm_m1 <- normalize(risk_metrics_m1)
norm_m2 <- normalize(risk_metrics_m2)
norm_m3 <- normalize(risk_metrics_m3)
norm_m4 <- normalize(risk_metrics_m4)
norm_m5 <- normalize(risk_metrics_m5)
...
EW_norm <- tibble(
  Metric = c("Sharp", "SD", "Downside Deviation", "VaR", "Inv HHI", "Entropy"),
  "Market 1" = c(norm_m1$EW, unlist(enc_m1$naive) %>% mean(), unlist(enb_m1$naive) %>% mean()),
  "Market 2" = c(norm_m2$EW, unlist(enc_m2$naive) %>% mean(), unlist(enb_m2$naive) %>% mean()),
  "Market 3" = c(norm_m3$EW, unlist(enc_m3$naive) %>% mean(), unlist(enb_m3$naive) %>% mean()),
  "Market 4" = c(norm_m4$EW, unlist(enc_m4$naive) %>% mean(), unlist(enb_m4$naive) %>% mean()),
  "Market 5" = c(norm_m5$EW, unlist(enc_m5$naive) %>% mean(), unlist(enb_m5$naive) %>% mean())
)
MV_norm <- tibble(
  Metric =  c("Sharp", "SD", "Downside Deviation", "VaR", "Inv HHI", "Entropy"),
  "Market 1" = c(norm_m1$MV, unlist(enc_m1$minvol) %>% mean(), unlist(enb_m1$minvol) %>% mean()),
  "Market 2" = c(norm_m2$MV, unlist(enc_m2$minvol) %>% mean(), unlist(enb_m2$minvol) %>% mean()),
  "Market 3" = c(norm_m3$MV, unlist(enc_m3$minvol) %>% mean(), unlist(enb_m3$minvol) %>% mean()),
  "Market 4" = c(norm_m4$MV, unlist(enc_m4$minvol) %>% mean(), unlist(enb_m4$minvol) %>% mean()),
  "Market 5" = c(norm_m5$MV, unlist(enc_m5$minvol) %>% mean(), unlist(enb_m5$minvol) %>% mean())
)
IV_norm <- tibble(
  Metric =  c("Sharp", "SD", "Downside Deviation", "VaR", "Inv HHI", "Entropy"),
  "Market 1" = c(norm_m1$IV, unlist(enc_m1$invvol) %>% mean(), unlist(enb_m1$invvol) %>% mean()),
  "Market 2" = c(norm_m2$IV, unlist(enc_m2$invvol) %>% mean(), unlist(enb_m2$invvol) %>% mean()),
  "Market 3" = c(norm_m3$IV, unlist(enc_m3$invvol) %>% mean(), unlist(enb_m3$invvol) %>% mean()),
  "Market 4" = c(norm_m4$IV, unlist(enc_m4$invvol) %>% mean(), unlist(enb_m4$invvol) %>% mean()),
  "Market 5" = c(norm_m5$IV, unlist(enc_m5$invvol) %>% mean(), unlist(enb_m5$invvol) %>% mean())
)
ERC_norm <- tibble(
  Metric =  c("Sharp", "SD", "Downside Deviation", "VaR", "Inv HHI", "Entropy"),
  "Market 1" = c(norm_m1$ERC, unlist(enc_m1$erc) %>% mean(), unlist(enb_m1$erc) %>% mean()),
  "Market 2" = c(norm_m2$ERC, unlist(enc_m2$erc) %>% mean(), unlist(enb_m2$erc) %>% mean()),
  "Market 3" = c(norm_m3$ERC, unlist(enc_m3$erc) %>% mean(), unlist(enb_m3$erc) %>% mean()),
  "Market 4" = c(norm_m4$ERC, unlist(enc_m4$erc) %>% mean(), unlist(enb_m4$erc) %>% mean()),
  "Market 5" = c(norm_m5$ERC, unlist(enc_m5$erc) %>% mean(), unlist(enb_m5$erc) %>% mean())
)
MD_norm <- tibble(
  Metric =  c("Sharp", "SD", "Downside Deviation", "VaR", "Inv HHI", "Entropy"),
  "Market 1" = c(norm_m1$MD, unlist(enc_m1$maxdiv) %>% mean(), unlist(enb_m1$maxdiv) %>% mean()),
  "Market 2" = c(norm_m2$MD, unlist(enc_m2$maxdiv) %>% mean(), unlist(enb_m2$maxdiv) %>% mean()),
  "Market 3" = c(norm_m3$MD, unlist(enc_m3$maxdiv) %>% mean(), unlist(enb_m3$maxdiv) %>% mean()),
  "Market 4" = c(norm_m4$MD, unlist(enc_m4$maxdiv) %>% mean(), unlist(enb_m4$maxdiv) %>% mean()),
  "Market 5" = c(norm_m5$MD, unlist(enc_m5$maxdiv) %>% mean(), unlist(enb_m5$maxdiv) %>% mean())
)

norm_table <- list(EW = EW_norm,
                   MV = MV_norm,
                   IV = IV_norm,
                   ERC = ERC_norm,
                   MD = MD_norm)
save(norm_table, file = "data/norm_table.rda")
