...
market_1 <- 
    1:N %>%
    map_dfr(~sim_market_with_progress(corr = corr_1,
                                      k = 300,
                                      mv_dist = "t",
                                      mv_df = 3,
                                      marginal_dist = "norm",
...
market_1 <- market_1 %>% mutate(date = as.Date(date),
                                Universe = as.character(Universe))
dat <- market_1 %>% filter(Universe==1)
...
rets_m2 <- methods %>% 
  map(~port_rets_universal(market_2,
                           method = .x,
                           rebdates = rebdates,
                           look_back = 100))
names(rets_m2) <- c("naive", "minvol", "invvol", "erc", "maxdiv")
...
rets_m3 <- methods %>% 
  map(~port_rets_universal(market_3,
                           method = .x,
                           rebdates = rebdates,
                           look_back = 100))
names(rets_m3) <- c("naive", "minvol", "invvol", "erc", "maxdiv")
...
# Market 4
rets_m4 <- methods %>% 
  map(~port_rets_universal(market_4,
                           method = .x,
                           rebdates = rebdates,
                           look_back = 100))
names(rets_m4) <- c("naive", "minvol", "invvol", "erc", "maxdiv")
...
# Market 5
rets_m5 <- methods %>% 
  map(~port_rets_universal(market_5,
                           method = .x,
                           rebdates = rebdates,
                           look_back = 100))
names(rets_m5) <- c("naive", "minvol", "invvol", "erc", "maxdiv")
