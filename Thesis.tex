...
\title{Evaluating Portfolio Optimisters with Simulated Markets
Exhibiting Left-Tail Dependence}
...
\hypertarget{a-review-of-portfolio-construction-algorithms}{%
\subsection{A Review of Portfolio Construction
Algorithms}\label{a-review-of-portfolio-construction-algorithms}}

\hypertarget{introduction-1}{%
\subsubsection{Introduction}\label{introduction-1}}

Since Harry Markovitz's (1952) seminal work on mean variance portfolios
scholars around the globe have been aspiring to develop a robust
algorithm capable of \textbf{\emph{ex ante}} situating a portfolio on
the efficient frontier
(\protect\hyperlink{ref-Markovitz}{\textbf{Markovitz?}}). There are now
a wide array of alternatives, with many more in the making.

These optimisers however are not without their flaws. Mean variance
optimisers rely heavily on the accuracy of return forecasts, where small
errors often lead to vastly different returns
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}). Furthermore, quadratic
...
error if the covariance matrix suffers from a high condition
number\footnote{A condition number is defined as the absolute value of
  the ratio between a covariance matrix's largest and smallest
  eigenvalues (\protect\hyperlink{ref-lopez}{\textbf{lopez?}})}
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}};
\protect\hyperlink{ref-lopez2012}{\textbf{lopez2012?}}). Diagonal
matrices have the smallest condition number which increases as more
correlated variables are added. A small change in correlation within any
given entry can greatly alter the inverse of a high conditional number
matrix, \textbf{which in turn can greatly effect the portfolio weights}
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}). For a given sample
size, larger co-variance matrices are prone to more noise in estimation.
This si essentially due to a reduction in degrees of freedom as a sample
of at least \(1/2N(N+1)\) independent and identically distributed (iid)
observations are required to estimate an \(N\times N\) covariance
matrix. This issues is exacerbated by the fact that a markets covariance
structure tends to vary over time
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}).

Due to the sensitivity issues surrounding errors in the expected return
estimation, this work will only discuss the portfolio optimisation
techniques that forego this input. These include the naive equal weight,
inverse variance, hierarchical risk parity, ERC and the minimum variance
portfolios. \emph{The theoretical underpinnings of each will be reviewed
as well as their relative performance in historical back tests}.
...
garnered allot of attention for themselves, largely to their tendency to
achieve surprisingly high returns in historical back tests
(\protect\hyperlink{ref-clarke2011}{\textbf{clarke2011?}}). This
performance has been attributed to the surprising fact that both low
volatility stocks tend to earn returns in excess of the market, and that
high beta stocks tend not to be rewarded by higher returns
(\protect\hyperlink{ref-clark2011}{\textbf{clark2011?}};
\protect\hyperlink{ref-fama1992}{\textbf{fama1992?}}). These minimum
variance portfolios tend to achieve cumulative returns equal to or
slightly greater than market portfolio's whilst maintaining consistently
lower variance and achieving a noticeable improvement in downside risk
mitigation in times of crisis
(\protect\hyperlink{ref-clarke}{\textbf{clarke?}}).

\emph{Who developed the min variance portfolio??}

The weight attributed to an given security in the long-only
minimum-variance portfolio is given in Eq. \ref{minvar_eq}
(\protect\hyperlink{ref-clarke}{\textbf{clarke?}}).
...
\tightlist
\item
  \(\sigma^2_{LMV}\) is the ex ante return variance of the long-only
  minimum variance portfolio.
\item
  \(\sigma^2_{\epsilon i}\) is the ex ante idiosyncratic return variance
  for security i.
\item
...
The simplicity of Eq. \ref{minvar_eq} can be attributed to the
simplifying assumption of single index risk and the expected returns
being absent from the objective function. Eq. \ref{minvar_eq} indicates
that asset weights depend on two portfolio specific risk parameters, the
ex ante variance of the long-only portfolio and the long only threshold
beta, and two security specific risk parameters, the market beta and the
idiosyncratic return variance
(\protect\hyperlink{ref-clarke}{\textbf{clarke?}}). High market beta and
idiosyncratic variance reduce an assets optimal weight towards zero,
while a market beta greater than long-only threshold results in a
optimal weight of zero (\protect\hyperlink{ref-clark}{\textbf{clark?}}).
Interestingly empirical results indicate that the long-only minimum
variance portfolio tends to apply a zero weight to approximately 80\% of
investable securities.

\textbf{mention first rule of investing??}

\hypertarget{equally-weighted-risk-contributions-erc}{%
\subsubsection{Equally-Weighted Risk Contributions
(ERC)}\label{equally-weighted-risk-contributions-erc}}

The ERC portfolio is another optimiser that avoids using expected
returns and is therefore said to be more robust
(\protect\hyperlink{ref-maillard}{\textbf{maillard?}}). The basic idea
behind the ERC is to weight the portfolio such that each security
contributes equally to risk
(\protect\hyperlink{ref-maillard}{\textbf{maillard?}}). The ERC
portfolio \(x=(x_1,x_2,...,x_n)\) consisting of n assets is designed as
follows:
...
portfolio optimisers, (\protect\hyperlink{ref-lopez}{\textbf{lopez?}})
developed a new approach incorporating machine-learning methods and
graph theory (\protect\hyperlink{ref-arevalo}{\textbf{arevalo?}}).
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}) argues that the ``lack
of hierarchical structure in a correlation matrix allows weights to vary
freely in unintended way'' and that this contributes to the instability
...
definite property (\protect\hyperlink{ref-lopex}{\textbf{lopex?}}). This
procedure works in three stages:

\begin{enumerate}
\def\labelenumi{\arabic{enumi})}
\item
  Tree clustering: This involves utilizing a hierarchical tree
  clustering method such that security weights can be determined
  downstream through a tree graph
  (\protect\hyperlink{ref-lopez}{\textbf{lopez?}}).
\item
  Quasi-diagonalization: this stage reorganizes the covariances matrix
  so that similar securities are placed near one another while
  dissimilar securities are far apart. This is done through an algorithm
  that situates the largest covariances near the diagonal and smaller
  covariances further away
  (\protect\hyperlink{ref-lopez}{\textbf{lopez?}}).
\item
  Recursive bisection: Portfolio funds are then allocated top down
  through the tree structure such that riskier clusters receive fewer
  funds. This can be accomplished through the inverse-variance
  allocation (\protect\hyperlink{ref-lopez}{\textbf{lopez?}}).
\end{enumerate}

Note that each of the three steps \emph{(or just step 1 and 3??)} can be
completed with an range of alternative techniques, which in turn effect
the solution.

(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}) carried out an in
sample simulation study comparing the respective allocations of the
long-only minimum variance, IVP and HRP portfolios using a covariance
matrix using a condition number that is ``not unfavourable'' to the
minimum variance portfolio. The simulated data consisted of 10000
observations across 10 variables. The following findings were made: The
minimum variance portfolio concentrated 92.66\% of funds in the top 5
holdings and assigned a zero weight to 3 assets. \emph{Conversly}, HRP
only assigned 62.5\% of its funds to the top 5 holdings
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}). The minimum variance
...
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}). This apparent
...
(\protect\hyperlink{ref-lopez}{\textbf{lopez?}}). This claim was further
